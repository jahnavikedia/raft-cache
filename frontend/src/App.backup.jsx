    ...
    const interval = setInterval(async () => {
      const updatedNodes = await Promise.all(nodes.map(async (node) => {
        try {
          ...
          return { ...node, ...res.data, active: true }
        } catch (e) {
      ...
      const leader = updatedNodes.find(n => n.state === 'LEADER' && n.active)
      if (leader) {
        try {
          // Note: Real implementation would need a log endpoint, 
          // for now we'll simulate or fetch from stats if available
          // For demo, we might need to add a log endpoint to backend or just show stats
        } catch (e) {
    ...
    <div className="container">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <div className="card">
          ...
          <MLStats prediction={mlPrediction} setPrediction={setMlPrediction} />
        </div>
      </div>
    </div>
  )
